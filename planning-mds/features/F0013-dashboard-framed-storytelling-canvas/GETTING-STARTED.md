...
- The Vite dev proxy currently targets `http://localhost:5113` for API routes via `experience/vite.config.ts`.
- No F0013-specific environment variables are required.
...
| Frontend | `experience/src/features/opportunities/components/ConnectedFlow.tsx` | Vertical timeline, stage nodes, branch rendering |
